...
<template>
  <Combobox v-model="selectedValue" :multiple="multiple" nullable v-slot="{ open: isComboboxOpen }">
    <Popover class="w-full" v-model:show="showOptions" ref="rootRef" :placement="placement">
      ...
      <template #body="{ isOpen, togglePopover }">
        <div v-show="isOpen">
          <div class="relative mt-1 rounded-lg bg-surface-modal text-base shadow-2xl" :class="bodyClasses">
            <ComboboxOptions
              class="max-h-[15rem] overflow-y-auto px-1.5 pb-1.5"
              :class="{ 'pt-1.5': hideSearch }"
              static
            >
              <div v-if="!hideSearch" class="sticky top-0 z-10 flex items-stretch space-x-1.5 bg-surface-modal py-1.5">
                ...
                >
                  ...
                  >
                    <div class="flex flex-1 gap-2 overflow-hidden items-center">
                      <div v-if="$slots['item-prefix'] || props.multiple" class="flex flex-shrink-0">
                        <slot name="item-prefix" v-bind="{ active, selected, option }">
                          <FeatherIcon name="check" v-if="isOptionSelected(option)" class="h-4 w-4 text-ink-gray-7" />
                    ...
                    <div v-if="$slots['item-suffix'] || option?.description" class="ml-2 flex-shrink-0">
              ...
              <li v-if="groups.length == 0" class="rounded-md px-2.5 py-1.5 text-base text-ink-gray-5">
                No results found
              </li>
            </ComboboxOptions>
            ...
            <div v-if="$slots.footer || props.showFooter || multiple" class="border-t p-1">
              <slot name="footer" v-bind="{ togglePopover }">
                <div v-if="multiple" class="flex items-center justify-end">
                  <Button v-if="!areAllOptionsSelected" label="Select All" @click.stop="selectAll" />
...
const isOptionSelected = (option: AutocompleteOption) => {
  ...
  return (selectedValue.value as AutocompleteOption[]).find((v) => (isOption(v) ? v.value === value : v === value))
}
